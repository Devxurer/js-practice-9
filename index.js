...
function getPositiveArrayValues(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let transition = Array.from(iterator);

    let newArray = transition.filter((value) => value > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  if (!Array.isArray(arr)) {
    ...
    arr.splice(startIdx);
    let doubledArray = arr.map((value) => value * 2);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let boardNumber = squareArray.filter((value) => value > threshold);
...
function capitalizeWords(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    let capitalizedArray = arr.map(
      (value, index) => value[0].toUpperCase() + value.slice(1)
    );
